...
ECHO.
ECHO ##################################################
ECHO # Running: ruff check %AGNO_INFRA_DIR%
ECHO ##################################################
ECHO.

python -m ruff check "%AGNO_INFRA_DIR%"
IF %ERRORLEVEL% NEQ 0 (
...
ECHO.
ECHO ##################################################
ECHO # Running: mypy %AGNO_INFRA_DIR% --config-file %AGNO_INFRA_DIR%\pyproject.toml
ECHO ##################################################
ECHO.

python -m mypy "%AGNO_INFRA_DIR%" --config-file "%AGNO_INFRA_DIR%\pyproject.toml"
IF %ERRORLEVEL% NEQ 0 (
    ECHO [ERROR] mypy validation failed with exit code %ERRORLEVEL%
    EXIT /B %ERRORLEVEL%
)

ECHO [INFO] agno_infra validation complete.
EXIT /B 0
