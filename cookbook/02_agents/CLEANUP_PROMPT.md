Goal: Clean up and standardize `cookbook/02_agents` so all Python examples pass the structure checker and use current model conventions.
...
1. Run the structure checker and fix all violations:
   ```
   .venvs/demo/bin/python cookbook/scripts/check_cookbook_pattern.py --base-dir cookbook/02_agents --recursive
   ```
   Common fixes:
