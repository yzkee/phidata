...
Simulates what an external API client would do:
1. Agent pauses (approval created).
2. List all pending approvals (with filters).
3. Get a specific approval by ID.
4. Resolve it (approve or reject).
5. Verify resolution and clean up.

Run: .venvs/demo/bin/python cookbook/02_agents/human_in_the_loop/approvals/approval_list_and_resolve.py
"""
...
for a in approvals:
    print(f"  [{a['id'][:8]}...] run={a['run_id'][:8]}... context={a.get('context')}")
