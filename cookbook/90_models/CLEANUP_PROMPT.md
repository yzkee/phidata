Goal: Clean up and standardize `cookbook/90_models` so all Python examples pass the structure checker.
...
Note: This directory contains provider-specific model examples. Do NOT migrate model classes
...
1. Run the structure checker and fix all violations:
   ```
   .venvs/demo/bin/python cookbook/scripts/check_cookbook_pattern.py --base-dir cookbook/90_models --recursive
   ```
   Common fixes:
