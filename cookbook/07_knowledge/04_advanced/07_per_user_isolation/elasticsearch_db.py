"""
Per-User Isolation: Elasticsearch
=================================
...
Elasticsearch keeps the owner in a top-level user_id keyword field. A scoped read
matches that term OR "must_not exists user_id", so documents written before the
field existed read as shared content. The scope is applied inside the knn clause,
which pre-filters: post-filtering would return nothing when the nearest neighbors
all belong to another owner.
...
vector_db = Elasticsearch(index_name=INDEX_NAME, url=ELASTICSEARCH_URL)
...
knowledge = Knowledge(
    name="per_user_demo",
    description="Per-user RAG isolation demo (Elasticsearch)",
    vector_db=vector_db,
)
