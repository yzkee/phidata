...
2. API keys (OPENAI_API_KEY, X/Twitter credentials)
3. Agent import
...
def check_api_keys() -> bool:
    ...
    if x_bearer:
        print(f"   [OK] X_BEARER_TOKEN is set ({x_bearer[:8]}...)")
    elif twitter_bearer:
        print(f"   [OK] TWITTER_BEARER_TOKEN is set ({twitter_bearer[:8]}...)")
    else:
        print("   [FAIL] X/Twitter credentials not set")
        print("   -> Run: export X_BEARER_TOKEN=your-token")
...
def check_import() -> bool:
    ...
    try:
        ...
        print("   [OK] social_media_agent imported successfully")
        return True
...
def main() -> int:
    ...
    print("=" * 60)
    print("Social Media Analyst Tutorial - Setup Check")
    print("=" * 60)
